...
//
// const outputTvBrands = inventory.map((inventory) => {listBrands.innerHTML += `<li>${inventory.brand}</li>`});
...
function generateListTvBrands (arrayTvs) {
    const listBrands = document.getElementById("list-brands");
    arrayTvs.map((tv) => {listBrands.innerHTML += `<li>${tv.brand}</li>`});
}
